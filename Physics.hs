...
updateParticle w p = p {_x = _x p + cos rot * v, _y = _y p + sin rot * v, _rot = rot, _near = nearC}
  where
    ps     = _particles w
...
hemisphere center near
  ...
  | rel > pi  = -1 -- right
  | otherwise = 0
  where rel = norm $ angleBetween2P (_x center) (_y center) (_x near) (_y near) - _rot center
